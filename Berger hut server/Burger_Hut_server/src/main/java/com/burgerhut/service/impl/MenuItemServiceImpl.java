...
public class MenuItemServiceImpl implements MenuItemService {
    ...
    public void deleteMenuItem(Long id) {
        ...
        if (!orderItems.isEmpty()) {
            // Optionally, set the menu item as unavailable (soft delete)
            MenuItem menuItem = menuItemRepository.findById(id).get();
            menuItem.setAvailable(false); // Mark as unavailable
            menuItemRepository.save(menuItem); // Save updated item
        } else {
